...
func (ph *productHandler) HighlightProduct(ctx context.Context, request *product.HighlightProductRequest) (*product.HighlightProductResponse, error) {
	validationErrors, err := utils.CheckValidation(request)
	if err != nil {
		return nil, err
	}
	...
	res, err := ph.productService.HighlightProduct(ctx)
	if err != nil {
		return nil, err
	}
